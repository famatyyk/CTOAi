...
def test_module_contract_requires_loader_registry_global_and_return():
    ...
    diagnostics_source = (OTCLIENT_DIR / "ctoa_helper_diagnostics.lua").read_text(encoding="utf-8")
    for function_name in [
        "boolText",
        "posText",
        "hasApi",
    ...
    hotkeys_source = (OTCLIENT_DIR / "ctoa_helper_hotkeys.lua").read_text(encoding="utf-8")
    for function_name in ["trim", "normalizeKeyName", "parse", "normalize", "isAllowed", "bindingDecision", "display", "actionbarSlotText", "contract"]:
    ...
    combat_runtime_source = (OTCLIENT_DIR / "ctoa_helper_combat_runtime.lua").read_text(encoding="utf-8")
    for function_name in ["plan", "summary", "adapterSummary", "magicSummary", "msLeftText", "runeReady", "rotationSpellRows", "spellReadiness", "rotationSpell", "offensiveAction", "actionStatusText", "targetingStatusText", "nextActionText", "waitReason", "decisionState", "decisionStateSummary", "contract"]:
    ...
    for function_name in ["request", "isPending", "isExpired", "confirm", "cancel", "decision", "decisionText", "statusText", "buttonText", "contract"]:
    ...
    for function_name in ["startText", "disarmedText", "position", "state", "visibilityText", "runtimeText", "uiSummary", "operatorSummary", "contract"]:
    ...
    for function_name in ["position", "label", "posKey", "add", "clear", "select", "delete", "move", "retryStatus", "retryBlocked", "progress", "stats", "selectedSummary", "uiState", "deleteRequest", "contract"]:
    ...
    targeting_source = (OTCLIENT_DIR / "ctoa_helper_targeting.lua").read_text(encoding="utf-8")
    for function_name in ["normalizedName", "isIgnoredName", "hasBlockingNpcIcon", "creatureTypeDecision", "priorityRank", "scoreCandidate", "bestCandidate", "decision", "summary", "configSummary", "contract"]:
    ...
    combat_runtime_source = (OTCLIENT_DIR / "ctoa_helper_combat_runtime.lua").read_text(encoding="utf-8")
    for function_name in ["plan", "summary", "adapterSummary", "magicSummary", "msLeftText", "runeReady", "rotationSpellRows", "spellReadiness", "rotationSpell", "offensiveAction", "actionStatusText", "targetingStatusText", "nextActionText", "contract"]:
        assert f"function CombatRuntime.{function_name}" in combat_runtime_source
    assert "owns_runtime_plan = true" in combat_runtime_source
    assert "owns_adapter_summary = true" in combat_runtime_source
    assert "owns_magic_summary = true" in combat_runtime_source
    ...
    cavebot_runtime_source = (OTCLIENT_DIR / "ctoa_helper_cavebot_runtime.lua").read_text(encoding="utf-8")
    for function_name in ["plan", "summary", "decisionText", "adapterSummary", "adapterStatusText", "adapterStatusSummary", "movementCapability", "probeSnapshot", "probeSummary", "probeReport", "pathText", "movementBlockedReason", "walkPreflight", "testWalkPlan", "walkingStatus", "retryDecision", "statusText", "traceText", "contract"]:
    ...
    loot_runtime_source = (OTCLIENT_DIR / "ctoa_helper_loot_runtime.lua").read_text(encoding="utf-8")
    ...
    timer_runtime_source = (OTCLIENT_DIR / "ctoa_helper_timer_runtime.lua").read_text(encoding="utf-8")
    ...
    profile_schema_source = (OTCLIENT_DIR / "ctoa_helper_profile_schema.lua").read_text(encoding="utf-8")
    for function_name in [
        "requiredSections",
        "sectionOrder",
        "safeFalseKeys",
    ...
    profile_persistence_source = (OTCLIENT_DIR / "ctoa_helper_profile_persistence.lua").read_text(encoding="utf-8")
    for function_name in [
        "profileCandidates",
        "uiPrefsCandidates",
        "saveDefaults",
        ...
        assert f"function ProfilePersistence.{function_name}" in profile_persistence_source
    ...
    operator_summary_source = (OTCLIENT_DIR / "ctoa_helper_operator_summary.lua").read_text(encoding="utf-8")
    for function_name in [
        "title",
        "healing",
        "healFriend",
    ...
    planner_source = (OTCLIENT_DIR / "ctoa_helper_planner.lua").read_text(encoding="utf-8")
    ...
    runtime_policy_source = (OTCLIENT_DIR / "ctoa_helper_runtime_policy.lua").read_text(encoding="utf-8")
    for function_name in ["requiredGates", "protectionZonePolicy", "resolvedProtectionZonePolicy", "protectionZoneDecision", "snapshot", "decision", "summary", "contract"]:
    ...
    dispatch_guard_source = (OTCLIENT_DIR / "ctoa_helper_dispatch_guard.lua").read_text(encoding="utf-8")
    ...
    plan_queue_source = (OTCLIENT_DIR / "ctoa_helper_plan_queue.lua").read_text(encoding="utf-8")
    ...
    readiness_source = (OTCLIENT_DIR / "ctoa_helper_runtime_readiness.lua").read_text(encoding="utf-8")
    for function_name in ["requiredComponents", "requiredGates", "snapshot", "decision", "summary", "contract"]:
    ...
    module_status_source = (OTCLIENT_DIR / "ctoa_helper_module_status.lua").read_text(encoding="utf-8")
    for function_name in ["defaultOrder", "normalize", "snapshot", "summary", "contract"]:
    ...
    action_catalog_source = (OTCLIENT_DIR / "ctoa_helper_action_catalog.lua").read_text(encoding="utf-8")
    for function_name in ["requiredGates", "all", "domains", "byAction", "classify", "summary", "contract"]:
    ...
    decision_trace_source = (OTCLIENT_DIR / "ctoa_helper_decision_trace.lua").read_text(encoding="utf-8")
    ...
    sandbox_handoff_source = (OTCLIENT_DIR / "ctoa_helper_sandbox_handoff.lua").read_text(encoding="utf-8")
    ...
    assert "solteria_helper_test_env.ps1 -Action SmokeAttachModules" in sandbox_handoff_source
    assert "solteria_helper_test_env.ps1 -Action SmokeAttachAll" in sandbox_handoff_source
    ...
    feature_flags_source = (OTCLIENT_DIR / "ctoa_helper_feature_flags.lua").read_text(encoding="utf-8")
    for function_name in ["all", "safeFalseKeys", "byKey", "audit", "summary", "toolsSummary", "contract"]:
    ...
    heal_friend_source = (OTCLIENT_DIR / "ctoa_helper_heal_friend.lua").read_text(encoding="utf-8")
    ...
    conditions_source = (OTCLIENT_DIR / "ctoa_helper_conditions.lua").read_text(encoding="utf-8")
...
def test_module_contract_blocks_forbidden_passive_actions(tmp_path: Path):
    ...
    for path in OTCLIENT_DIR.glob("ctoa_helper_*.lua"):
        (otclient_dir / path.name).write_text(path.read_text(encoding="utf-8"), encoding="utf-8")
...
def test_module_contract_writes_json_and_markdown(tmp_path: Path):
    ...
    contract.write_json_atomic(json_out, json.loads(json.dumps(report, default=lambda value: value.__dict__)))
