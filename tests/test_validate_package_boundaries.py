...
def test_validate_package_boundaries_detects_parent_private_path_overlap(tmp_path: Path):
    core_manifest = tmp_path / "core.manifest.json"
    pro_manifest = tmp_path / "pro.manifest.json"
    studio_manifest = tmp_path / "studio.manifest.json"
    ...
    core_payload["include_paths"] = list(core_payload.get("include_paths", [])) + ["archived"]
    ...
    assert any("private path overlap" in issue for issue in issues)
