...
def test_next_modules_plan_is_safe_and_ordered():
    ...
    assert "remaining runtime pressure at combat/cavebot adapters" in modules[4]["source_basis"]
    ...
    assert modules[-1]["status"] == "source_required"
    ...
    assert "movement blocked-reason/status/trace/path result text" in supplemental[2]["current_slice"]
...
def test_vbot_import_review_keeps_source_required_contract():
    review = (plan.ROOT / "docs" / "otclient" / "vbot_import_review.md").read_text(encoding="utf-8")

    assert "Status: `source_required`" in review
    assert "No vBot source tree or reviewed local archive is present" in review
    ...
    assert "Do not directly copy code" in review
...
def test_next_modules_plan_markdown_calls_out_runtime_blockers():
    ...
    assert "ctoa_helper_operator_summary.lua owns title/domain/profile/UI summary composition" in markdown
    ...
    assert "`profile_schema` / Profile schema, persistence policy, rotation metadata, and migration metadata | `static_gated`" in markdown
...
def test_next_modules_plan_writes_atomic_json_and_markdown(tmp_path: Path):
    ...
    assert "otclient_input_contract_fixtures.py" in saved["supplemental_execution"][8]["current_slice"]
