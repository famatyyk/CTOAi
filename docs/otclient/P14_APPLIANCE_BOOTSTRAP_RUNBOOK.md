...
disabled, set the fixed endpoint profile, and create the named snapshot expected
by the host runner. The runner resolves the snapshot UUID from that predeclared
name at runtime, so this bootstrap does not require a post-provision source
change or a hand-entered snapshot UUID. The prior NAT-capable snapshot is not
