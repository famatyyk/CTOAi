...
def _safe_repo_relative_path(path_str: str) -> Path:
    ...
    if resolved != root_resolved and root_resolved not in resolved.parents:
        raise ValueError("Deliverable path escapes repository root")
    if candidate.exists() and candidate.is_symlink():
        raise ValueError("Deliverable path must not be a symlink")
    return candidate
...
def write_deliverable(path_str: str, title: str, body: str) -> Path:
    ...
    path = _safe_repo_relative_path(path_str)
...
def invoke_llm_for_task(
    task_id: str, prompt: str, context: Dict[str, Any] = None
) -> Optional[str]:
...
"""
        runbook = runbook.replace("{timestamp}", now_iso(), 1)
...
"""
        checklist = checklist.replace("{timestamp}", now_iso(), 1)
