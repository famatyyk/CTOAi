...
from runner.http_safety import require_public_discovery_url
...
def _safe_template_component(value: str, label: str) -> str:
    cleaned = str(value or "").strip()
    if (
        not cleaned
        or cleaned in {".", ".."}
        or any(ord(char) < 32 for char in cleaned)
        or any(char in _WINDOWS_UNSAFE_FILENAME_CHARS for char in cleaned)
        or "/" in cleaned
        or "\\" in cleaned
    ):
...
def _require_template_source_url(url: str) -> str:
    value = require_public_discovery_url(url, label="Template source").rstrip("/")
    parsed = urllib.parse.urlparse(value)
    if parsed.username or parsed.password or parsed.query or parsed.fragment:
        raise ValueError(
            "Template source URL must not include credentials, query, or fragment"
        )
    if "\\" in parsed.path:
        raise ValueError("Template source URL path must not include backslashes")
    path_parts = [urllib.parse.unquote(part) for part in parsed.path.split("/")]
    if any(part in {".", ".."} for part in path_parts):
        raise ValueError("Template source URL path must not contain traversal")
    return value
...
class TemplateLibrary:
    ...
    def __init__(
        ...
        self.server_url = (
            _require_template_source_url(server_url) if server_url else None
        )
    ...
    def load_creatures(self, creature_names: List[str]) -> int:
        ...
        for name in creature_names:
            try:
                ...
                if template:
                    self.creatures[template.name] = template
                    loaded += 1
    ...
    def _load_creature(self, name: str) -> Optional[Template]:
        ...
        safe_name = _safe_template_component(name, "template name")
        # Try local cache first
        cache_file = self._cache_file("creature", safe_name)
        if cache_file.exists():
            return self._load_from_disk(cache_file, "creature", safe_name)

        # Try server API
        if self.server_url:
            try:
                url_name = urllib.parse.quote(safe_name, safe="")
                return self._load_from_server(
                    f"{self.server_url}/creatures/{url_name}",
                    "creature",
                    safe_name,
                )
            except Exception as e:
                log.debug(f"Server load failed for {safe_name}: {e}")

        # Fallback: create blank template (to be filled during gameplay)
        log.warning(
            f"No template found for creature '{safe_name}'; creating placeholder"
        )
    ...
    def load_minimap_sections(
        self, world_bounds: Tuple[int, int, int, int], section_size: int = 100
    ) -> int:
        """
        ...
        for x in range(min_x, max_x, section_size):
            for y in range(min_y, max_y, section_size):
                ...
                try:
                    # Try cache first
                    cache_file = self._cache_file("minimap", f"{x}_{y}_7")
                    if cache_file.exists():
                        template = self._load_from_disk(
                            cache_file, "minimap", coord_name
                        )
                    # Try server API
                    elif self.server_url:
                        template = self._load_from_server(
                            f"{self.server_url}/minimap/{x}/{y}", "minimap", coord_name
                        )
                    else:
                        template = None
    ...
    def _cache_file(self, tpl_type: str, name: str) -> Path:
        """Build a cache file path that cannot escape the configured cache dir."""
        safe_type = _safe_template_component(tpl_type, "template type")
        safe_name = _safe_template_component(name, "template name")
        cache_file = self.cache_dir / f"{safe_type}_{safe_name}.png"
        cache_root = self.cache_dir.resolve()
        resolved = cache_file.resolve()
        if resolved != cache_root and cache_root not in resolved.parents:
            raise ValueError("Template cache file must stay under cache directory")
        return cache_file

    def _load_from_disk(
        self, path: Path, tpl_type: str, name: str
    ) -> Optional[Template]:
    ...
    def _load_from_server(
        self, url: str, tpl_type: str, name: str
    ) -> Optional[Template]:
        """Load template from remote server."""
        try:
            safe_url = _require_template_source_url(url)
            # _require_template_source_url rejects secret-bearing or ambiguous URLs.
            response = urlopen(safe_url, timeout=5)  # nosec B310
            ...
            if self.use_cache:
                cache_file = self._cache_file(tpl_type, name)
            ...
            log.error(f"Failed to load template from server: {e}")
            return None

    def save_template(self, template: Template) -> bool:
        """Cache template to disk."""
        try:
            cache_file = self._cache_file(template.template_type, template.name)
