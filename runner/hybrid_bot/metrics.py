...
class MetricsCollector:
    ...
    def record_snapshot(
        ...
        supplies_per_hour = (
            supplies_cost_gold / duration_hours if duration_hours > 0 else 0
        )
    ...
    def get_session_summary(self) -> SessionMetrics:
        ...
        if not self.snapshots:
            return SessionMetrics(
                session_id=self.session_id, start_time=utc_now().isoformat()
            )
    ...
    def get_location_stats(self, location: str) -> dict:
        ...
        return {
            ...
            "average_xp_per_hour": sum(s.xp_per_hour for s in location_snapshots)
            / len(location_snapshots),
            "average_balance_per_hour": sum(
                s.balance_per_hour for s in location_snapshots
            )
            / len(location_snapshots),
...
def compare_with_manual_metrics(
    ...
    """
    xp_ratio = (
        bot_metrics.average_xp_per_hour / manual_xp_per_hour
        if manual_xp_per_hour > 0
        else 0
    )
    balance_ratio = (
        bot_metrics.average_balance_per_hour / manual_balance_per_hour
        if manual_balance_per_hour > 0
        else 0
    )
