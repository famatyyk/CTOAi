...
function Assert-P14BootstrapLogonCleanup {
    if (
        -not (Test-Path -LiteralPath $P14BootstrapLogonCleanupReceiptPath -PathType Leaf) -or
        (Test-P14ReparsePoint $P14BootstrapLogonCleanupReceiptPath)
    ) {
        Stop-P14GuestProvision 'bootstrap_logon_cleanup_receipt_missing'
    }
    $receipt = Get-P14Json $P14BootstrapLogonCleanupReceiptPath
    ...
    if (
        ...
        Stop-P14GuestProvision 'bootstrap_logon_cleanup_receipt_invalid'
    }
    if (Get-ScheduledTask -TaskName $P14BootstrapLogonCleanupTaskName -ErrorAction SilentlyContinue) {
        Stop-P14GuestProvision 'bootstrap_logon_cleanup_task_present'
    }
    try {
        if (
            ...
            Stop-P14GuestProvision 'bootstrap_logon_state_not_cleared'
        }
    } catch {
...
function Assert-P14GuestPrerequisites {
    ...
    try {
        ...
        Stop-P14GuestProvision 'dedicated_operator_lookup_failed'
    }
    if (
