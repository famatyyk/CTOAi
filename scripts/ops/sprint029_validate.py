...
REQUIRED_FILES = ['workflows/backlog-sprint-029.yaml', 'workflows/sprint-029-delivery-flow.yaml', 'scripts/ops/sprint029_validate.py', '.vscode/tasks.json', '.github/workflows/ctoa-pipeline.yml', 'runtime/experiments/sprint-029/CTOA-143.md', 'runtime/experiments/sprint-029/CTOA-144.md', 'runtime/experiments/sprint-029/CTOA-145.md', 'runtime/experiments/sprint-029/CTOA-146.md', 'runtime/experiments/sprint-029/CTOA-147.md']
REQUIRED_YAML_FILES = ['workflows/backlog-sprint-029.yaml', 'workflows/sprint-029-delivery-flow.yaml']
...
def check_missing_hooks(root: Path) -> dict:
    flow_path = root / "workflows" / "sprint-029-delivery-flow.yaml"
    try:
...
def check_pipeline_gate(root: Path) -> dict:
    ...
    gate_present = "scripts/ops/sprint029_validate.py" in content
    artifact_present = "runtime/ci-artifacts/sprint-029-validation.json" in content
...
def check_local_tasks(root: Path) -> dict:
    ...
    labels_present = all(
        needle in content
        for needle in [
            "CTOA: Sprint-029 Validate",
            "CTOA: Sprint-029 Wave-1 Run",
        ]
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Sprint-029 validator")
    ...
    if args.json_out:
        ...
        print(f"[sprint029_validate] Report written to {out_path}")

    print(f"[sprint029_validate] {report['status']} — {report['summary']}")
