...
REQUIRED_FILES = ['workflows/backlog-sprint-035.yaml', 'workflows/sprint-035-delivery-flow.yaml', 'scripts/ops/sprint035_validate.py', '.vscode/tasks.json', '.github/workflows/ctoa-pipeline.yml', 'runtime/experiments/sprint-035/CTOA-173.md', 'runtime/experiments/sprint-035/CTOA-174.md', 'runtime/experiments/sprint-035/CTOA-175.md', 'runtime/experiments/sprint-035/CTOA-176.md', 'runtime/experiments/sprint-035/CTOA-177.md']
REQUIRED_YAML_FILES = ['workflows/backlog-sprint-035.yaml', 'workflows/sprint-035-delivery-flow.yaml']
...
def check_missing_hooks(root: Path) -> dict:
    flow_path = root / "workflows" / "sprint-035-delivery-flow.yaml"
    try:
...
def check_pipeline_gate(root: Path) -> dict:
    ...
    gate_present = "scripts/ops/sprint035_validate.py" in content
    artifact_present = "runtime/ci-artifacts/sprint-035-validation.json" in content
...
def check_local_tasks(root: Path) -> dict:
    ...
    labels_present = all(
        needle in content
        for needle in [
            "CTOA: Sprint-035 Validate",
            "CTOA: Sprint-035 Wave-1 Run",
        ]
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Sprint-035 validator")
    ...
    if args.json_out:
        ...
        print(f"[sprint035_validate] Report written to {out_path}")

    print(f"[sprint035_validate] {report['status']} — {report['summary']}")
