...
REQUIRED_FILES = [
    'workflows/backlog-sprint-042.yaml',
    'workflows/sprint-042-delivery-flow.yaml',
...
def check_missing_hooks(root: Path) -> dict:
    flow_path = root / "workflows" / "sprint-042-delivery-flow.yaml"
    try:
...
def check_pipeline_gate(root: Path) -> dict:
    ...
    gate_present = "scripts/ops/sprint042_validate.py" in content
    artifact_present = "runtime/ci-artifacts/sprint-042-validation.json" in content
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Sprint-042 validator")
    ...
    if args.json_out:
        ...
        print(f"[sprint042_validate] Report written to {out_path}")

    print(f"[sprint042_validate] {report['status']} - {report['summary']}")
    ...
    if failed_ids:
        print(f"[sprint042_validate] failed checks: {', '.join(failed_ids)}")
