...
REQUIRED_FILES = [
    'workflows/backlog-sprint-043.yaml',
...
def check_missing_hooks(root: Path) -> dict:
    flow_path = root / "workflows" / "sprint-043-delivery-flow.yaml"
    try:
...
def check_pipeline_gate(root: Path) -> dict:
    ...
    gate_present = "scripts/ops/sprint043_validate.py" in content
    artifact_present = "runtime/ci-artifacts/sprint-043-validation.json" in content
...
def check_local_tasks(root: Path) -> dict:
    ...
    labels_present = all(
        needle in content
        for needle in [
            "CTOA: Sprint-043 Validate",
            "CTOA: Sprint-043 Wave-1 Run",
        ]
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Sprint-043 validator")
    ...
    if args.json_out:
        ...
        print(f"[sprint043_validate] Report written to {out_path}")

    print(f"[sprint043_validate] {report['status']} - {report['summary']}")
    ...
    if failed_ids:
        print(f"[sprint043_validate] failed checks: {', '.join(failed_ids)}")
