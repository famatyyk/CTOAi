...
REQUIRED_FILES = [
    'workflows/backlog-sprint-046.yaml',
    'workflows/sprint-046-delivery-flow.yaml',
...
def check_missing_hooks(root: Path) -> dict:
    flow_path = root / "workflows" / "sprint-046-delivery-flow.yaml"
    try:
...
def check_pipeline_gate(root: Path) -> dict:
    ...
    gate_present = "scripts/ops/sprint046_validate.py" in content
    artifact_present = "runtime/ci-artifacts/sprint-046-validation.json" in content
...
def check_local_tasks(root: Path) -> dict:
    ...
    labels_present = all(
        needle in content
        for needle in [
            "CTOA: Sprint-046 Validate",
            "CTOA: Sprint-046 Wave-1 Run",
            "CTOA: Sprint-046 Refresh Progress Diagram",
        ]
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Sprint-046 validator")
    ...
    if args.json_out:
        ...
        print(f"[sprint046_validate] Report written to {out_path}")

    print(f"[sprint046_validate] {report['status']} - {report['summary']}")
    ...
    if failed_ids:
        print(f"[sprint046_validate] failed checks: {', '.join(failed_ids)}")
