...
REQUIRED_FILES = ['workflows/backlog-sprint-038.yaml', 'workflows/sprint-038-delivery-flow.yaml', 'scripts/ops/sprint038_validate.py', '.vscode/tasks.json', '.github/workflows/ctoa-pipeline.yml', 'runtime/experiments/sprint-038/CTOA-188.md', 'runtime/experiments/sprint-038/CTOA-189.md', 'runtime/experiments/sprint-038/CTOA-190.md', 'runtime/experiments/sprint-038/CTOA-191.md', 'runtime/experiments/sprint-038/CTOA-192.md']
REQUIRED_YAML_FILES = ['workflows/backlog-sprint-038.yaml', 'workflows/sprint-038-delivery-flow.yaml']
...
def check_missing_hooks(root: Path) -> dict:
    flow_path = root / "workflows" / "sprint-038-delivery-flow.yaml"
    try:
...
def check_pipeline_gate(root: Path) -> dict:
    ...
    gate_present = "scripts/ops/sprint038_validate.py" in content
    artifact_present = "runtime/ci-artifacts/sprint-038-validation.json" in content
...
def check_local_tasks(root: Path) -> dict:
    ...
    labels_present = all(
        needle in content
        for needle in [
            "CTOA: Sprint-038 Validate",
            "CTOA: Sprint-038 Wave-1 Run",
        ]
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Sprint-038 validator")
    ...
    if args.json_out:
        ...
        print(f"[sprint038_validate] Report written to {out_path}")

    print(f"[sprint038_validate] {report['status']} — {report['summary']}")
