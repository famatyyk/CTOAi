...
REQUIRED_FILES = [
    'workflows/backlog-sprint-056.yaml',
    'workflows/sprint-056-delivery-flow.yaml',
    'docs/history/sprints/SPRINT-056.md',
...
def check_missing_hooks(root: Path) -> dict:
    flow_path = root / "workflows" / "sprint-056-delivery-flow.yaml"
    try:
...
def check_pipeline_gate(root: Path) -> dict:
    ...
    gate_present = "scripts/ops/sprint056_validate.py" in content
    artifact_present = "runtime/ci-artifacts/sprint-056-validation.json" in content
    summary_present = "runtime/ci-artifacts/sprint-056-wave1-summary.txt" in content
...
def check_local_tasks(root: Path) -> dict:
    ...
    labels_present = all(
        needle in content
        for needle in [
            "CTOA: Sprint-056 Validate",
            "CTOA: Sprint-056 Wave-1 Run",
            "CTOA: Sprint-056 Refresh Progress Diagram",
            "CTOA: Sprint-056 Quality Snapshot",
            "CTOA: Sprint-056 State Sync",
            "CTOA: Sprint-056 Wave Summary UTF-8",
        ]
    )
...
def check_state_evidence_alignment(root: Path) -> dict:
    sprint_doc_path = root / "docs/history/sprints/SPRINT-056.md"
    backlog_path = root / "workflows/backlog-sprint-056.yaml"
    ...
    if state.get("backlog_id") != "sprint-056":
        return {
            "id": "state_evidence_alignment",
            "ok": False,
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Sprint-056 validator")
    ...
    if args.json_out:
        ...
        print(f"[sprint056_validate] Report written to {out_path}")

    print(f"[sprint056_validate] {report['status']} - {report['summary']}")
    ...
    if failed_ids:
        print(f"[sprint056_validate] failed checks: {', '.join(failed_ids)}")
