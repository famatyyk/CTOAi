...
REQUIRED_FILES = [
    'workflows/backlog-sprint-047.yaml',
    'workflows/sprint-047-delivery-flow.yaml',
...
def check_missing_hooks(root: Path) -> dict:
    flow_path = root / "workflows" / "sprint-047-delivery-flow.yaml"
    try:
...
def check_pipeline_gate(root: Path) -> dict:
    ...
    gate_present = "scripts/ops/sprint047_validate.py" in content
    artifact_present = "runtime/ci-artifacts/sprint-047-validation.json" in content
...
def check_local_tasks(root: Path) -> dict:
    ...
    labels_present = all(
        needle in content
        for needle in [
            "CTOA: Sprint-047 Validate",
            "CTOA: Sprint-047 Wave-1 Run",
            "CTOA: Sprint-047 Refresh Progress Diagram",
        ]
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Sprint-047 validator")
    ...
    if args.json_out:
        ...
        print(f"[sprint047_validate] Report written to {out_path}")

    print(f"[sprint047_validate] {report['status']} - {report['summary']}")
    ...
    if failed_ids:
        print(f"[sprint047_validate] failed checks: {', '.join(failed_ids)}")
