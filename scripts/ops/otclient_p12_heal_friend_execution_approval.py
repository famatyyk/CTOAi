#!/usr/bin/env python3
"""Add separate P12 Heal Friend execution approval; never cast or send chat."""
...
if __package__:
    ...
    from . import otclient_p12_heal_friend_session_approval as sessions
else:  # pragma: no cover
    import otclient_p12_heal_friend_execute_once_plan as plans
    import otclient_p12_heal_friend_session_approval as sessions


def build_execution_approval(
    plan: dict[str, Any], approval: dict[str, Any], confirmation: str, *, now_ms: int
) -> dict[str, Any]:
    blockers: list[str] = []
    if (
        approval.get("schema_version") != sessions.SCHEMA
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        return 1
    result = build_execution_approval(
        plan, approval, args.confirmation, now_ms=int(time.time() * 1000)
    )
