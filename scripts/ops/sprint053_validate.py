...
def check_missing_hooks(root: Path) -> dict:
    flow_path = root / "workflows" / "sprint-053-delivery-flow.yaml"
    try:
...
def check_pipeline_gate(root: Path) -> dict:
    ...
    gate_present = "scripts/ops/sprint053_validate.py" in content
    artifact_present = "runtime/ci-artifacts/sprint-053-validation.json" in content
...
def check_local_tasks(root: Path) -> dict:
    ...
    labels_present = all(
        needle in content
        for needle in [
            "CTOA: Sprint-053 Validate",
            "CTOA: Sprint-053 Wave-1 Run",
            "CTOA: Sprint-053 Refresh Progress Diagram",
            "CTOA: Sprint-053 Quality Snapshot",
            "CTOA: Sprint-053 State Sync",
        ]
    )
...
def check_state_evidence_alignment(root: Path) -> dict:
    sprint_doc_path = root / "docs/history/sprints/SPRINT-053.md"
    backlog_path = root / "workflows/backlog-sprint-053.yaml"
    ...
    if state.get("backlog_id") != "sprint-053":
        return {
            "id": "state_evidence_alignment",
            "ok": False,
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Sprint-053 validator")
    ...
    if args.json_out:
        ...
        print(f"[sprint053_validate] Report written to {out_path}")

    print(f"[sprint053_validate] {report['status']} - {report['summary']}")
    ...
    if failed_ids:
        print(f"[sprint053_validate] failed checks: {', '.join(failed_ids)}")
