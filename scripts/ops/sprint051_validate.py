...
REQUIRED_FILES = [
    'workflows/backlog-sprint-051.yaml',
    'workflows/sprint-051-delivery-flow.yaml',
...
def check_missing_hooks(root: Path) -> dict:
    flow_path = root / "workflows" / "sprint-051-delivery-flow.yaml"
    try:
...
def check_pipeline_gate(root: Path) -> dict:
    ...
    gate_present = "scripts/ops/sprint051_validate.py" in content
    artifact_present = "runtime/ci-artifacts/sprint-051-validation.json" in content
...
def check_local_tasks(root: Path) -> dict:
    ...
    labels_present = all(
        needle in content
        for needle in [
            "CTOA: Sprint-051 Validate",
            "CTOA: Sprint-051 Wave-1 Run",
            "CTOA: Sprint-051 Refresh Progress Diagram",
            "CTOA: Sprint-051 Quality Snapshot",
        ]
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Sprint-051 validator")
    ...
    if args.json_out:
        ...
        print(f"[sprint051_validate] Report written to {out_path}")

    print(f"[sprint051_validate] {report['status']} - {report['summary']}")
    ...
    if failed_ids:
        print(f"[sprint051_validate] failed checks: {', '.join(failed_ids)}")
