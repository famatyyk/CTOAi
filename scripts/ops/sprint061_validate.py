...
REQUIRED_FILES = [
    'workflows/backlog-sprint-061.yaml',
    'workflows/sprint-061-delivery-flow.yaml',
    'docs/history/sprints/SPRINT-061.md',
...
def check_missing_hooks(root: Path) -> dict:
    flow_path = root / "workflows" / "sprint-061-delivery-flow.yaml"
    try:
...
def check_pipeline_gate(root: Path) -> dict:
    ...
    gate_present = "scripts/ops/sprint061_validate.py" in content
    artifact_present = "runtime/ci-artifacts/sprint-061-validation.json" in content
    summary_present = "runtime/ci-artifacts/sprint-061-wave1-summary.txt" in content
    progress_present = "docs/history/sprints/SPRINT-061-PROGRESS.md" in content
    ok = gate_present and artifact_present and summary_present and progress_present

    hint_parts: list[str] = []
    if not gate_present:
        hint_parts.append("add sprint-061 validator command")
    if not artifact_present:
        hint_parts.append("add sprint-061 validation artifact path")
    if not summary_present:
        hint_parts.append("add sprint-061 summary artifact path")
    if not progress_present:
        hint_parts.append("add sprint-061 progress artifact path")
...
def check_local_tasks(root: Path) -> dict:
    ...
    labels_present = all(
        needle in content
        for needle in [
            "CTOA: Sprint-061 Validate",
            "CTOA: Sprint-061 Refresh Progress Diagram",
            "CTOA: Sprint-061 Quality Snapshot",
            "CTOA: Sprint-061 State Sync",
            "CTOA: Sprint-061 Wave Summary UTF-8",
            "CTOA: Sprint-061 Wave-1 Run",
        ]
    )
    ...
    return {
        "id": "local_tasks",
        "ok": labels_present,
        "hint": "Add sprint-061 validate, progress, quality, state sync, wave summary, and wave-1 tasks" if not labels_present else "",
    }


def check_state_evidence_alignment(root: Path) -> dict:
    sprint_doc_path = root / "docs/history/sprints/SPRINT-061.md"
    backlog_path = root / "workflows/backlog-sprint-061.yaml"
    ...
    if state.get("backlog_id") != "sprint-061":
        return {
            "id": "state_evidence_alignment",
            "ok": False,
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Sprint-061 validator")
    ...
    if args.json_out:
        ...
        print(f"[sprint061_validate] Report written to {out_path}")

    print(f"[sprint061_validate] {report['status']} - {report['summary']}")
    ...
    if failed_ids:
        print(f"[sprint061_validate] failed checks: {', '.join(failed_ids)}")
