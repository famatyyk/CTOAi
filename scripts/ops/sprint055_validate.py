...
def check_missing_hooks(root: Path) -> dict:
    flow_path = root / "workflows" / "sprint-055-delivery-flow.yaml"
    try:
...
def check_pipeline_gate(root: Path) -> dict:
    ...
    gate_present = "scripts/ops/sprint055_validate.py" in content
    artifact_present = "runtime/ci-artifacts/sprint-055-validation.json" in content
    summary_present = "runtime/ci-artifacts/sprint-055-wave1-summary.txt" in content
...
def check_local_tasks(root: Path) -> dict:
    ...
    labels_present = all(
        needle in content
        for needle in [
            "CTOA: Sprint-055 Validate",
            "CTOA: Sprint-055 Wave-1 Run",
            "CTOA: Sprint-055 Refresh Progress Diagram",
            "CTOA: Sprint-055 Quality Snapshot",
            "CTOA: Sprint-055 State Sync",
            "CTOA: Sprint-055 Wave Summary UTF-8",
        ]
    )
...
def check_state_evidence_alignment(root: Path) -> dict:
    sprint_doc_path = root / "docs/history/sprints/SPRINT-055.md"
    backlog_path = root / "workflows/backlog-sprint-055.yaml"
    ...
    if state.get("backlog_id") != "sprint-055":
        return {
            "id": "state_evidence_alignment",
            "ok": False,
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Sprint-055 validator")
    ...
    if args.json_out:
        ...
        print(f"[sprint055_validate] Report written to {out_path}")

    print(f"[sprint055_validate] {report['status']} - {report['summary']}")
    ...
    if failed_ids:
        print(f"[sprint055_validate] failed checks: {', '.join(failed_ids)}")
