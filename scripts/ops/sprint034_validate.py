...
REQUIRED_FILES = ['workflows/backlog-sprint-034.yaml', 'workflows/sprint-034-delivery-flow.yaml', 'scripts/ops/sprint034_validate.py', '.vscode/tasks.json', '.github/workflows/ctoa-pipeline.yml', 'runtime/experiments/sprint-034/CTOA-168.md', 'runtime/experiments/sprint-034/CTOA-169.md', 'runtime/experiments/sprint-034/CTOA-170.md', 'runtime/experiments/sprint-034/CTOA-171.md', 'runtime/experiments/sprint-034/CTOA-172.md']
REQUIRED_YAML_FILES = ['workflows/backlog-sprint-034.yaml', 'workflows/sprint-034-delivery-flow.yaml']
...
def check_missing_hooks(root: Path) -> dict:
    flow_path = root / "workflows" / "sprint-034-delivery-flow.yaml"
    try:
...
def check_pipeline_gate(root: Path) -> dict:
    ...
    gate_present = "scripts/ops/sprint034_validate.py" in content
    artifact_present = "runtime/ci-artifacts/sprint-034-validation.json" in content
...
def check_local_tasks(root: Path) -> dict:
    ...
    labels_present = all(
        needle in content
        for needle in [
            "CTOA: Sprint-034 Validate",
            "CTOA: Sprint-034 Wave-1 Run",
        ]
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Sprint-034 validator")
    ...
    if args.json_out:
        ...
        print(f"[sprint034_validate] Report written to {out_path}")

    print(f"[sprint034_validate] {report['status']} — {report['summary']}")
