...
def check_missing_hooks(root: Path) -> dict:
    flow_path = root / "workflows" / "sprint-041-delivery-flow.yaml"
    try:
...
def check_pipeline_gate(root: Path) -> dict:
    ...
    gate_present = "scripts/ops/sprint041_validate.py" in content
    artifact_present = "runtime/ci-artifacts/sprint-041-validation.json" in content
