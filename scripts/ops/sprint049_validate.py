...
REQUIRED_FILES = [
    'workflows/backlog-sprint-049.yaml',
    'workflows/sprint-049-delivery-flow.yaml',
...
def check_missing_hooks(root: Path) -> dict:
    flow_path = root / "workflows" / "sprint-049-delivery-flow.yaml"
    try:
...
def check_pipeline_gate(root: Path) -> dict:
    ...
    gate_present = "scripts/ops/sprint049_validate.py" in content
    artifact_present = "runtime/ci-artifacts/sprint-049-validation.json" in content
...
def check_local_tasks(root: Path) -> dict:
    ...
    labels_present = all(
        needle in content
        for needle in [
            "CTOA: Sprint-049 Validate",
            "CTOA: Sprint-049 Wave-1 Run",
            "CTOA: Sprint-049 Refresh Progress Diagram",
            "CTOA: Sprint-049 Quality Snapshot",
        ]
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Sprint-049 validator")
    ...
    if args.json_out:
        ...
        print(f"[sprint049_validate] Report written to {out_path}")

    print(f"[sprint049_validate] {report['status']} - {report['summary']}")
    ...
    if failed_ids:
        print(f"[sprint049_validate] failed checks: {', '.join(failed_ids)}")
