...
REQUIRED_FILES = ['workflows/backlog-sprint-036.yaml', 'workflows/sprint-036-delivery-flow.yaml', 'scripts/ops/sprint036_validate.py', '.vscode/tasks.json', '.github/workflows/ctoa-pipeline.yml', 'runtime/experiments/sprint-036/CTOA-178.md', 'runtime/experiments/sprint-036/CTOA-179.md', 'runtime/experiments/sprint-036/CTOA-180.md', 'runtime/experiments/sprint-036/CTOA-181.md', 'runtime/experiments/sprint-036/CTOA-182.md']
REQUIRED_YAML_FILES = ['workflows/backlog-sprint-036.yaml', 'workflows/sprint-036-delivery-flow.yaml']
...
def check_missing_hooks(root: Path) -> dict:
    flow_path = root / "workflows" / "sprint-036-delivery-flow.yaml"
    try:
...
def check_pipeline_gate(root: Path) -> dict:
    ...
    gate_present = "scripts/ops/sprint036_validate.py" in content
    artifact_present = "runtime/ci-artifacts/sprint-036-validation.json" in content
...
def check_local_tasks(root: Path) -> dict:
    ...
    labels_present = all(
        needle in content
        for needle in [
            "CTOA: Sprint-036 Validate",
            "CTOA: Sprint-036 Wave-1 Run",
        ]
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Sprint-036 validator")
    ...
    if args.json_out:
        ...
        print(f"[sprint036_validate] Report written to {out_path}")

    print(f"[sprint036_validate] {report['status']} — {report['summary']}")
