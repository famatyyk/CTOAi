...
def check_missing_hooks(root: Path) -> dict:
    flow_path = root / "workflows" / "sprint-052-delivery-flow.yaml"
    try:
...
def check_pipeline_gate(root: Path) -> dict:
    ...
    gate_present = "scripts/ops/sprint052_validate.py" in content
    artifact_present = "runtime/ci-artifacts/sprint-052-validation.json" in content
...
def check_local_tasks(root: Path) -> dict:
    ...
    labels_present = all(
        needle in content
        for needle in [
            "CTOA: Sprint-052 Validate",
            "CTOA: Sprint-052 Wave-1 Run",
            "CTOA: Sprint-052 Refresh Progress Diagram",
            "CTOA: Sprint-052 Quality Snapshot",
            "CTOA: Sprint-052 State Sync",
        ]
    )
...
def check_state_evidence_alignment(root: Path) -> dict:
    sprint_doc_path = root / "docs/history/sprints/SPRINT-052.md"
    backlog_path = root / "workflows/backlog-sprint-052.yaml"
    ...
    if state.get("backlog_id") != "sprint-052":
        return {
            "id": "state_evidence_alignment",
            "ok": False,
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Sprint-052 validator")
    ...
    if args.json_out:
        ...
        print(f"[sprint052_validate] Report written to {out_path}")

    print(f"[sprint052_validate] {report['status']} - {report['summary']}")
    ...
    if failed_ids:
        print(f"[sprint052_validate] failed checks: {', '.join(failed_ids)}")
