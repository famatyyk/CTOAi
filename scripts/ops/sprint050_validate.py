...
REQUIRED_FILES = [
    'workflows/backlog-sprint-050.yaml',
    'workflows/sprint-050-delivery-flow.yaml',
    'docs/history/sprints/SPRINT-050.md',
...
def check_missing_hooks(root: Path) -> dict:
    flow_path = root / "workflows" / "sprint-050-delivery-flow.yaml"
    try:
...
def check_pipeline_gate(root: Path) -> dict:
    ...
    gate_present = "scripts/ops/sprint050_validate.py" in content
    artifact_present = "runtime/ci-artifacts/sprint-050-validation.json" in content
...
def check_local_tasks(root: Path) -> dict:
    ...
    labels_present = all(
        needle in content
        for needle in [
            "CTOA: Sprint-050 Validate",
            "CTOA: Sprint-050 Wave-1 Run",
            "CTOA: Sprint-050 Refresh Progress Diagram",
            "CTOA: Sprint-050 Quality Snapshot",
        ]
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Sprint-050 validator")
    ...
    if args.json_out:
        ...
        print(f"[sprint050_validate] Report written to {out_path}")

    print(f"[sprint050_validate] {report['status']} - {report['summary']}")
    ...
    if failed_ids:
        print(f"[sprint050_validate] failed checks: {', '.join(failed_ids)}")
