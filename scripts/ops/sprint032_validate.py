...
REQUIRED_FILES = ['workflows/backlog-sprint-032.yaml', 'workflows/sprint-032-delivery-flow.yaml', 'scripts/ops/sprint032_validate.py', '.vscode/tasks.json', '.github/workflows/ctoa-pipeline.yml', 'runtime/experiments/sprint-032/CTOA-158.md', 'runtime/experiments/sprint-032/CTOA-159.md', 'runtime/experiments/sprint-032/CTOA-160.md', 'runtime/experiments/sprint-032/CTOA-161.md', 'runtime/experiments/sprint-032/CTOA-162.md']
REQUIRED_YAML_FILES = ['workflows/backlog-sprint-032.yaml', 'workflows/sprint-032-delivery-flow.yaml']
...
def check_missing_hooks(root: Path) -> dict:
    flow_path = root / "workflows" / "sprint-032-delivery-flow.yaml"
    try:
...
def check_pipeline_gate(root: Path) -> dict:
    ...
    gate_present = "scripts/ops/sprint032_validate.py" in content
    artifact_present = "runtime/ci-artifacts/sprint-032-validation.json" in content
...
def check_local_tasks(root: Path) -> dict:
    ...
    labels_present = all(
        needle in content
        for needle in [
            "CTOA: Sprint-032 Validate",
            "CTOA: Sprint-032 Wave-1 Run",
        ]
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Sprint-032 validator")
    ...
    if args.json_out:
        ...
        print(f"[sprint032_validate] Report written to {out_path}")

    print(f"[sprint032_validate] {report['status']} — {report['summary']}")
