...
REQUIRED_FILES = ['workflows/backlog-sprint-031.yaml', 'workflows/sprint-031-delivery-flow.yaml', 'scripts/ops/sprint031_validate.py', '.vscode/tasks.json', '.github/workflows/ctoa-pipeline.yml', 'runtime/experiments/sprint-031/CTOA-153.md', 'runtime/experiments/sprint-031/CTOA-154.md', 'runtime/experiments/sprint-031/CTOA-155.md', 'runtime/experiments/sprint-031/CTOA-156.md', 'runtime/experiments/sprint-031/CTOA-157.md']
REQUIRED_YAML_FILES = ['workflows/backlog-sprint-031.yaml', 'workflows/sprint-031-delivery-flow.yaml']
...
def check_missing_hooks(root: Path) -> dict:
    flow_path = root / "workflows" / "sprint-031-delivery-flow.yaml"
    try:
...
def check_pipeline_gate(root: Path) -> dict:
    ...
    gate_present = "scripts/ops/sprint031_validate.py" in content
    artifact_present = "runtime/ci-artifacts/sprint-031-validation.json" in content
...
def check_local_tasks(root: Path) -> dict:
    ...
    labels_present = all(
        needle in content
        for needle in [
            "CTOA: Sprint-031 Validate",
            "CTOA: Sprint-031 Wave-1 Run",
        ]
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Sprint-031 validator")
    ...
    if args.json_out:
        ...
        print(f"[sprint031_validate] Report written to {out_path}")

    print(f"[sprint031_validate] {report['status']} — {report['summary']}")
