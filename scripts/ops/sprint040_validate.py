...
REQUIRED_FILES = [
    'workflows/backlog-sprint-040.yaml',
    'workflows/sprint-040-delivery-flow.yaml',
...
def _config() -> SprintValidatorConfig:
    return SprintValidatorConfig(
        ...
        flow_file='workflows/sprint-040-delivery-flow.yaml',
        pipeline_file='.github/workflows/ctoa-pipeline.yml',
        pipeline_validator_snippet='scripts/ops/sprint040_validate.py',
        pipeline_artifact_snippet='runtime/ci-artifacts/sprint-040-validation.json',
        required_task_labels=[
            'CTOA: Sprint-040 Validate',
            'CTOA: Sprint-040 Wave-1 Run',
        ],
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(description='Sprint-040 validator')
    ...
    if args.json_out:
        ...
        print(f'[sprint040_validate] Report written to {out_path}')

    print(f"[sprint040_validate] {report['status']} - {report['summary']}")
