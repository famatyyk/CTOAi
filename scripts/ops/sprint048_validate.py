...
REQUIRED_FILES = [
    'workflows/backlog-sprint-048.yaml',
    'workflows/sprint-048-delivery-flow.yaml',
    'docs/history/sprints/SPRINT-048.md',
...
def check_missing_hooks(root: Path) -> dict:
    flow_path = root / "workflows" / "sprint-048-delivery-flow.yaml"
    try:
...
def check_pipeline_gate(root: Path) -> dict:
    ...
    gate_present = "scripts/ops/sprint048_validate.py" in content
    artifact_present = "runtime/ci-artifacts/sprint-048-validation.json" in content
...
def check_local_tasks(root: Path) -> dict:
    ...
    labels_present = all(
        needle in content
        for needle in [
            "CTOA: Sprint-048 Validate",
            "CTOA: Sprint-048 Wave-1 Run",
            "CTOA: Sprint-048 Refresh Progress Diagram",
            "CTOA: Sprint-048 Quality Snapshot",
        ]
    )
    ...
    return {
        "id": "local_tasks",
        "ok": labels_present,
        "hint": "Add sprint validate, wave-1, progress diagram, and quality snapshot tasks" if not labels_present else "",
    }
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Sprint-048 validator")
    ...
    if args.json_out:
        ...
        print(f"[sprint048_validate] Report written to {out_path}")

    print(f"[sprint048_validate] {report['status']} - {report['summary']}")
    ...
    if failed_ids:
        print(f"[sprint048_validate] failed checks: {', '.join(failed_ids)}")
