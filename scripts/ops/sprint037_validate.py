...
REQUIRED_FILES = ['workflows/backlog-sprint-037.yaml', 'workflows/sprint-037-delivery-flow.yaml', 'scripts/ops/sprint037_validate.py', '.vscode/tasks.json', '.github/workflows/ctoa-pipeline.yml', 'runtime/experiments/sprint-037/CTOA-183.md', 'runtime/experiments/sprint-037/CTOA-184.md', 'runtime/experiments/sprint-037/CTOA-185.md', 'runtime/experiments/sprint-037/CTOA-186.md', 'runtime/experiments/sprint-037/CTOA-187.md']
REQUIRED_YAML_FILES = ['workflows/backlog-sprint-037.yaml', 'workflows/sprint-037-delivery-flow.yaml']
...
def check_missing_hooks(root: Path) -> dict:
    flow_path = root / "workflows" / "sprint-037-delivery-flow.yaml"
    try:
...
def check_pipeline_gate(root: Path) -> dict:
    ...
    gate_present = "scripts/ops/sprint037_validate.py" in content
    artifact_present = "runtime/ci-artifacts/sprint-037-validation.json" in content
...
def check_local_tasks(root: Path) -> dict:
    ...
    labels_present = all(
        needle in content
        for needle in [
            "CTOA: Sprint-037 Validate",
            "CTOA: Sprint-037 Wave-1 Run",
        ]
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Sprint-037 validator")
    ...
    if args.json_out:
        ...
        print(f"[sprint037_validate] Report written to {out_path}")

    print(f"[sprint037_validate] {report['status']} — {report['summary']}")
