...
def check_missing_hooks(root: Path) -> dict:
    flow_path = root / "workflows" / "sprint-054-delivery-flow.yaml"
    try:
...
def check_local_tasks(root: Path) -> dict:
    ...
    labels_present = all(
        needle in content
        for needle in [
            "CTOA: Sprint-054 Validate",
            "CTOA: Sprint-054 Wave-1 Run",
...
def check_state_evidence_alignment(root: Path) -> dict:
    sprint_doc_path = root / "docs/history/sprints/SPRINT-054.md"
    backlog_path = root / "workflows/backlog-sprint-054.yaml"
    ...
    if state.get("backlog_id") != "sprint-054":
        return {
            "id": "state_evidence_alignment",
            "ok": False,
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Sprint-054 validator")
    ...
    if args.json_out:
        ...
        print(f"[sprint054_validate] Report written to {out_path}")

    print(f"[sprint054_validate] {report['status']} - {report['summary']}")
    ...
    if failed_ids:
        print(f"[sprint054_validate] failed checks: {', '.join(failed_ids)}")
