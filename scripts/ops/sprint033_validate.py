...
REQUIRED_FILES = ['workflows/backlog-sprint-033.yaml', 'workflows/sprint-033-delivery-flow.yaml', 'scripts/ops/sprint033_validate.py', '.vscode/tasks.json', '.github/workflows/ctoa-pipeline.yml', 'runtime/experiments/sprint-033/CTOA-163.md', 'runtime/experiments/sprint-033/CTOA-164.md', 'runtime/experiments/sprint-033/CTOA-165.md', 'runtime/experiments/sprint-033/CTOA-166.md', 'runtime/experiments/sprint-033/CTOA-167.md']
REQUIRED_YAML_FILES = ['workflows/backlog-sprint-033.yaml', 'workflows/sprint-033-delivery-flow.yaml']
...
def check_missing_hooks(root: Path) -> dict:
    flow_path = root / "workflows" / "sprint-033-delivery-flow.yaml"
    try:
...
def check_pipeline_gate(root: Path) -> dict:
    ...
    gate_present = "scripts/ops/sprint033_validate.py" in content
    artifact_present = "runtime/ci-artifacts/sprint-033-validation.json" in content
...
def check_local_tasks(root: Path) -> dict:
    ...
    labels_present = all(
        needle in content
        for needle in [
            "CTOA: Sprint-033 Validate",
            "CTOA: Sprint-033 Wave-1 Run",
        ]
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Sprint-033 validator")
    ...
    if args.json_out:
        ...
        print(f"[sprint033_validate] Report written to {out_path}")

    print(f"[sprint033_validate] {report['status']} — {report['summary']}")
