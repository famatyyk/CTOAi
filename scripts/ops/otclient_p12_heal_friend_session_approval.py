#!/usr/bin/env python3
"""Persist P12 Heal Friend sandbox-session approval; never cast or send chat."""
...
OUTPUT = plans.DEV / "p12_heal_friend_session_approval.json"
SCHEMA = "ctoa.p12-heal-friend-session-approval.v1"


def build_approval(
    plan: dict[str, Any], confirmation: str, *, now_ms: int
) -> dict[str, Any]:
    blockers: list[str] = []
    if (
    ...
    if confirmation != plan.get("required_session_confirmation"):
    ...
    return {
        ...
        "approval_id": f"p12-heal-friend-session-{uuid.uuid4().hex[:16]}",
        ...
        "blockers": blockers,
        "lane": "heal_friend",
        "exact_vocation": plans.EXACT_VOCATION,
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        persisted = json.loads(plans.OUTPUT.read_text(encoding="utf-8"))
    except (OSError, ValueError, json.JSONDecodeError) as error:
        print(f"Invalid persisted plan: {error}", file=sys.stderr)
        return 1
    approval = build_approval(
        persisted, args.confirmation, now_ms=int(time.time() * 1000)
    )
