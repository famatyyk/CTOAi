...
REQUIRED_FILES = [
    'workflows/backlog-sprint-045.yaml',
    'workflows/sprint-045-delivery-flow.yaml',
    'docs/history/sprints/SPRINT-045.md',
...
def check_missing_hooks(root: Path) -> dict:
    flow_path = root / "workflows" / "sprint-045-delivery-flow.yaml"
    try:
...
def check_pipeline_gate(root: Path) -> dict:
    ...
    gate_present = "scripts/ops/sprint045_validate.py" in content
    artifact_present = "runtime/ci-artifacts/sprint-045-validation.json" in content
...
def check_local_tasks(root: Path) -> dict:
    ...
    labels_present = all(
        needle in content
        for needle in [
            "CTOA: Sprint-045 Validate",
            "CTOA: Sprint-045 Wave-1 Run",
            "CTOA: Sprint-045 Refresh Progress Diagram",
        ]
    )
    ...
    return {
        "id": "local_tasks",
        "ok": labels_present,
        "hint": "Add sprint validate, wave-1, and progress diagram tasks" if not labels_present else "",
    }
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Sprint-045 validator")
    ...
    if args.json_out:
        ...
        print(f"[sprint045_validate] Report written to {out_path}")

    print(f"[sprint045_validate] {report['status']} - {report['summary']}")
    ...
    if failed_ids:
        print(f"[sprint045_validate] failed checks: {', '.join(failed_ids)}")
